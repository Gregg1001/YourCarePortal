...
namespace YourCarePortal.Controllers
{
    public class CustomFormsController : Controller
    {
        ...
        public async Task<IActionResult> CustomForms()
        {
            ...
            // Fetch list of custom forms for the authenticated user
            // For demonstration purposes, using RootFormsListPerson instead of RootCustomForms
            ...
            string clientIDs = Authentication1?.LinkedClientIDS ?? "null";
        ...
        public async Task<IActionResult> FormsListAction(string id)
        {
            try
            {
                ...
                // Try deserializing the JSON string
                try
                {
                ...
                {
                    Console.WriteLine("Error deserializing the JSON string: " + jsonEx.Message);
        ...
        public async Task<IActionResult> FormsAction(int? clientId)
        {
            try
            {
                ...
                // Verify if helper initialization is successful
                if (!helper.IsInitialized)
                {
                    return View("NoData");
                }

                // Fetch clients based on the client ID
                ...
                var (root, debugMessage, jsonString) = await _responseHelper.GetResponse<RootFormsListPerson>(authKey, URL2);
