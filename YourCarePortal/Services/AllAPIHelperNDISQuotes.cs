...
namespace YourCarePortal.Services
{
    ...
    /// </summary>
    public class AllAPIHelperNDISQuotes
    {
        ...
        public AllAPIHelperNDISQuotes(HttpClient httpClient, string URL)
        {
        ...
        public async Task<RootNDISQuotes> SendRequest<RootNDISQuotes>(params KeyValuePair<string, string>[] parameters)
        {
            if (_apiUrl == null)
            {
            ...
            RootNDISQuotes result;

            // Attempt to deserialize the response content.
            try
            {
                result = JsonConvert.DeserializeObject<RootNDISQuotes>(responseBody);
                // result = System.Text.Json.JsonSerializer.Deserialize<RootNDISQuotes>(responseBody);
            }
            catch (Exception ex)
            {
                // Proper logging can be added here.
